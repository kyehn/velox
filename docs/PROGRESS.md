...
> 生成时间: 2026-05-28
> 代码行数: 5,297 行 Rust (不含测试)
> 测试数量: 214 个 (74 core + 7 android + 10 renderer + 123 terminal)
> Clippy 状态: 零警告
...
| `FontPipeline::glyph_info` 每次创建新实例 | 高 | 应复用 FontPipeline 实例，传入 &mut self |
| `build_cell_instances` 内部创建临时 FontPipeline | 高 | 应使用缓存的 glyph_info |
